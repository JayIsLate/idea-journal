...
import EntryIntro from "@/components/journal/EntryIntro";
...
export default function EntryView({ entry }: Props) {
  ...
  // The Abridged tab shows the rich structured summary stored in `abridged`.
  // Empty on first view → auto-generated by the effect below.
  const [summary, setSummary] = useState<string>(entry.abridged || "");
  // Regenerate only appears when content was added after the summary was made.
  // Cleared whenever we (re)generate, since the summary is then current.
  const [stale, setStale] = useState<boolean>(Boolean(entry.abridged_stale));
  ...
  return (
    <div className="md:flex md:gap-8">
      {/* One-time explainer on first entry view (after first submission) */}
      <EntryIntro />
      ...
      <div className="flex-1 min-w-0">
          ...
          >
            {hermesLoading
              ...
              : visibleHighlights.length > 0
                ? `Ponder (${visibleHighlights.length}) ↓`
                : "Ponder →"}
          </button>
        </div>
        ...
        <article className="pb-12">
          ...
          {view === "abridged" && summary && stale && (
            ...
            >
              {summarizing ? "Regenerating…" : "↻ Regenerate — entry was added to"}
            </button>
          )}
